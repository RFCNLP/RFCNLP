...
def token_features(token, pos, stem, vocab_size, pos_size, variable_ids, state_ids, event_ids, id2cap, id2word, n_token_pos, n_token_len, use_stem):
    vector = [0] * vocab_size
    vector_pos = [0] * pos_size
    vector_cap_pattern = [0] * 7
    ...
    timer_regex = r'.*timer.*'
    error_regex = r'.*error.*'
    state_regex = r'(^|\W)state($|\W)'

    variable_verbs = r'delet.*|set.*|sets|increment.*'
    action_verbs = r'send.*|receiv.*|issu.*|generat.*|form.*|creat.*'
    timer_verbs = r'start.*|restart.*|stop.*|reset.*'
    transition_verbs = r'enter.*|mov.*|chang.*|stay.*|leav.*|go.*|remain.*|.*\w\.state :=.*'
    transition_dir = r'to|from|through'
    ...
    if token is not None:
        if re.match(assignment_regex, id2word[token]):
            assignment_logic = 1
            #print(id2word[token])
        if re.match(comparison_regex, id2word[token]):
            comparison_logic = 1
        if re.match(math_regex, id2word[token]):
            math_logic = 1
        if re.match(timer_regex, id2word[token]):
            has_timer = 1
        if re.match(timer_verbs, id2word[token]):
            has_timer_verbs = 1
        if re.match(variable_verbs, id2word[token]):
            has_variable_verbs = 1
        if re.match(action_verbs, id2word[token]):
            has_action_verbs = 1
        if re.match(error_regex, id2word[token]):
            has_error = 1
        if token in variable_ids:
            has_variable = 1
        elif token in state_ids:
            ...
            has_event = 1
        elif re.match(state_regex, id2word[token]):
            has_state_regex = 1
        elif re.match(transition_verbs, id2word[token]):
            has_transition_verbs = 1
        elif re.match(transition_dir, id2word[token]):
            has_transition_dir = 1
        elif re.match(conditional_words, id2word[token]):
            has_conditional_words = 1
        ...
        vector_pos[pos] = 1
        vector_cap_pattern[id2cap[token]] = 1

    return vector + vector_pos + vector_cap_pattern + \
           [has_error, has_timer, has_variable, has_state, has_event,
            has_state_regex, has_transition_verbs, has_transition_dir,
            has_timer_verbs, has_variable_verbs, has_action_verbs, \
            assignment_logic, comparison_logic, math_logic, \
...
def transform_features_nochunk(X, vocab_size, pos_size, variable_ids, state_ids, event_ids, id2cap, id2word={}, use_stem=False):
    X_new = []
    for i, sequence in enumerate(X):
        x_sequence = []
        ...
        for n_token_pos, (token, pos, stem) in enumerate(zip(sequence[0], sequence[1], sequence[2])):
            ...
            # Get features for the currenta and the previous chunk
            curr_token_feats = token_features(token, pos, stem, vocab_size, pos_size, variable_ids, state_ids, event_ids, id2cap, id2word, n_token_pos + 1, n_seq_len, use_stem)
            prev_token_feats = token_features(previous_token, previous_pos, previous_stem, vocab_size, pos_size, variable_ids, state_ids, event_ids, id2cap, id2word, n_token_pos, n_seq_len, use_stem)
